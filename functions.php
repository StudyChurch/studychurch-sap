...
class SCV_Setup {
	...
	protected function add_filters() {
		...
		add_filter( 'sc_redirect_to_user_domain', '__return_false' );
	}
...
function checkout_setup() {
	 ...
	 */
	require_once( get_stylesheet_directory() . '/inc/admin/updater/theme-updater.php' );

	/**
	 * TGM activation class
	 */
	require_once( get_stylesheet_directory() . '/inc/admin/tgm/tgm-activation.php' );

	/**
	 * Load the Typekit class
	 */
	require_once( get_stylesheet_directory() . '/inc/typekit/typekit.php' );
	 ...
	 */
	require_once( get_stylesheet_directory() . '/inc/widgets/icon-select/icon-text.php' );

	/**
	 * Load the Gravatar profile widget
	 */
	require_once( get_stylesheet_directory() . '/inc/widgets/gravatar-profile.php' );

	/**
	 * Easy Digital Downloads functions
	 */
	if( class_exists( 'Easy_Digital_Downloads' ) ) {
		require_once( get_stylesheet_directory() . '/inc/edd/edd.php' );
	}
	 ...
	 */
	load_theme_textdomain( 'checkout', get_stylesheet_directory() . '/languages' );
...
function checkout_scripts() {
	 ...
	 */
	wp_enqueue_style( 'checkout-fontawesome-css', get_stylesheet_directory_uri() . "/inc/fonts/fontawesome/css/font-awesome.css", array( 'checkout-style' ), '4.3.0' );
	 ...
	 */
	wp_enqueue_script( 'checkout-custom-js', get_stylesheet_directory_uri() . '/js/checkout.js', array( 'jquery' ), '1.0', true );
	 ...
	 */
	wp_enqueue_script( 'checkout-match-height-js', get_stylesheet_directory_uri() . '/js/jquery.matchHeight.js', array(), '0.5.2', true );

	/**
	 * Enqueue ResponsiveSlides script
	 */
	wp_enqueue_script( 'checkout-slides-js', get_stylesheet_directory_uri() . '/js/responsiveslides.js', array( 'jquery' ), '1.54', true );

	/**
	 * Enqueue TouchSwipe
	 */
	wp_enqueue_script( 'checkout-swipe-js', get_stylesheet_directory_uri() . '/js/jquery.touchSwipe.js', array( 'jquery' ), '1.6.6', true );

	/**
	 * Enqueue FastClick
	 */
	wp_enqueue_script( 'checkout-fastclick-js', get_stylesheet_directory_uri() . '/js/fastclick.js', array( 'jquery' ), '1.06', true );

	/**
	 * Enqueue LightGallery script
	 */
	wp_enqueue_script( 'checkout-lightgallery-js', get_stylesheet_directory_uri() . '/js/light-gallery/js/lightGallery.js', array( 'jquery' ), '1.1.4', true );

	/**
	 * Enqueue LightGallery stylesheet
	 */
	wp_enqueue_style( 'checkout-venobox-css', get_stylesheet_directory_uri() . "/js/light-gallery/css/lightGallery.css", array( 'checkout-style' ), '1.1.4' );

	/**
	 * Enqueue FitVids
	 */
	wp_enqueue_script( 'checkout-fitvids-js', get_stylesheet_directory_uri() . '/js/jquery.fitvids.js', array(), '1.1', true );
	...
	if( class_exists( 'Easy_Digital_Downloads' ) && function_exists( 'EDD_FES' ) && is_page( EDD_FES()->helper->get_option( 'fes-vendor-dashboard-page', false ) ) ) {
		...
		if( isset( $_GET['task'] ) && 'products' === $_GET['task'] ) {
			wp_enqueue_script( 'checkout-cookie-js', get_stylesheet_directory_uri() . '/js/jquery.cookie.js', array(), '1.4.1', true );
		}
	}
...
function checkout_admin_fonts( $hook_suffix ) {
	...
	wp_enqueue_style( 'checkout-fontawesome-admin-css', get_stylesheet_directory_uri() . "/inc/fonts/fontawesome/css/font-awesome.css", array(), '4.3.0' );
}
