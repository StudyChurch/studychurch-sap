...
function checkout_icon_widget_scripts( $hook ) {
	...
	wp_enqueue_style( 'checkout-chosen', get_stylesheet_directory_uri() . "/inc/widgets/icon-select/chosen.css", array(), '1.3.0', 'screen' );
	wp_enqueue_script( 'checkout-chosen-script', get_stylesheet_directory_uri() . "/inc/widgets/icon-select/chosen.jquery.js", array( 'jquery' ), '1.3.0' );
	wp_enqueue_script( 'checkout-admin-widget', get_stylesheet_directory_uri() . "/inc/widgets/icon-select/icon-text.js", array( 'jquery' ), '1.0' );
}
	...
	public function form( $instance ) {
		...
		<p>
			...
			<select class="widefat icon-select" name="<?php echo esc_attr( $this->get_field_name( 'icon_select' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'icon_select' ) ); ?>" data-placeholder="<?php _e( 'Select an icon', 'checkout' ); ?>" style="font-family: FontAwesome">
				<?php
				$fontawesome = checkout_fontawesome( get_stylesheet_directory_uri() . '/inc/fonts/fontawesome/css/font-awesome.css' );
				ksort( $fontawesome );
